...
namespace RARIndia.DataAccessLayer.Repository
{
    public class RARIndiaViewRepository<T>
    {
        #region Declarations
        ...
        public RARIndiaViewRepository()
        {
            _context = HelperMethods.CurrentContext;
        }

        public RARIndiaViewRepository(IDBContext context)
        {
            _context = context;
        }
        ...
        public IList<T> ExecuteStoredProcedureList(string commandText, int storedProcedureTimeout, int? indexOutParamater, out int totalRowCount)
        {
            ...
            {
                RARIndiaFileLogging.LogObject(typeof(T), commandText, ex);
                throw;
            }
            finally
            {
